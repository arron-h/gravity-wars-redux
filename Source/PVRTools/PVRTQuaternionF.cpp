	...
	const float			fAngle)
{
	...
	fSin = (float)PVRTSIN(fAngle * 0.5f);
	fCos = (float)PVRTCOS(fAngle * 0.5f);
	...
	float					&fAngle)
{
	...
	fAngle		= (float)PVRTACOS(fCosAngle)*2.0f;
	...
	const float				t)
{
	...
	A = (float)(PVRTSIN((1.0f-t)*fAngle) / PVRTSIN(fAngle));
	B = (float)(PVRTSIN(t*fAngle) / PVRTSIN(fAngle));
